...
PROV-O 互換のリネージ、対話型オフラインワークベンチ、および読み取り専用の
ローカル MCP サーバーを組み合わせています。決定論的アナライザーと MCP サーバーは、
...
- 登録済みワークスペースを 7 個の読み取り専用ローカル MCP ツールから照会します。
- 一般的な Java ポリシーアクセサーの読み取りを、それが制御する制御フロー分岐へ
  マッピングします。任意の文字列リテラルは保持しません。
- 明示的な要求があった場合、最新のオントロジースナップショットとシャドーイングされていない
  ローカルポリシーから、作成専用かつモード `0400` の AETHER Lab ランタイム
  バインディングレシートを作成します。

バージョン 0.3.3 では、変更されたリポジトリを全面的に再解析します。フィンガープリントにより、
...
### オプションの AETHER Lab ランタイムバインディング

このローカル CLI 操作は、意図的に読み取り専用 MCP サーバーへ公開していません。
バージョン 0.3.3 がこのモード `0400` のレシートをサポートするのは macOS/POSIX であり、
...
因果関係の証明、正しさの保証ではありません。runtime-binding receipt が限定するのは、静的な
