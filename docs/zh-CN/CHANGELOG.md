...
- 为完整和公开 Skills-only 发布归档添加精确、可复现的验证，包括解压后的 smoke 检查。
...
- 添加从 Java `PolicyLeaf` 到 `RuntimeBranch` 的静态数据流边，同时不保留任意字符串字面量。
- 添加经过明确授权、仅限本地的精确 `aether.runtime-effective-ontology-binding/v1` 不可变回执生成器。
- 对陈旧/被篡改图谱、仅测试或未使用路径、已知活动阶梯遮蔽、已禁用 trailing 配置、歧义路径和已存在输出采用失败关闭。
- 说明运行时绑定证据是静态可达性，而非运行时、订单、安全性或利润因果证明。
