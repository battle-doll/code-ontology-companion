...
- 支持的源码变更后执行授权刷新，检查新鲜度和警告，并保留未支持及混合语言范围。
- Context 限于选定且确认的记录及回读，Contracts 验证选定的真实交换资料。
- 验证两项技能的软件包和解压运行。在检查点收集证据后再次核对新鲜度与快照身份，避免遗漏验证期间的变更。

## 0.6.0 - 2026-09-05

- 为庆祝 GPT-6 Astra 发布，精简检索指令并区分面向人的 3D 界面与面向 AI 的数据，不宣称未测量的模型性能提升或官方合作。
- 默认显示基于真实源码的有界 3D 地图，提供结构、影响、变更、镜头聚焦、证据选择、深链接、键盘与文本操作及安全的平面替代视图。
- 增加精确符号优先检索、结构过滤、分页、固定快照、按方向的依赖路径，以及节点、关系和证据修改比较。
- 保留证据 ID，保守处理被遮蔽的类型名与未解析调用。在安全读取和内容校验后复用有界索引。
- 增加原生 Code 引用与 Context 不可变 locator。真实快照仍不支持严格的 Contracts draft 转换，与合成兼容性测试明确区分。
- 从已提交的公开仓库生成带源码版本和校验和的自身本体演示，私有注册信息不发布。
- 扩充提取、检索、界面交互、互操作测试并更新可复现发布包。实际 Astra A/B 评估尚未测量。
