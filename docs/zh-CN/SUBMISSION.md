...
- 版本：0.7.0
- 候选状态：新的发布与提交草案；0.7.0 尚未获批或发布。
- 发布历史：2026-08-29 在 OpenAI Platform 确认 0.5.3 为 **Published**。这项历史确认不代表 0.7.0 已获批。
...
0.7.0 在 `manage-code-ontology` 之外增加 `apply-code-ontology`。它在当前对话中复用已授权工作区，执行状态检查及相关检索或影响查询。优先使用实际公开的 MCP；不可用时使用同一软件包内经过验证的 CLI。区分安装、技能公开、MCP公开、CLI可运行性，以及计划与已执行结果。
...
生成 ZIP 包含 manifest、技能、脚本、参考文档、许可证、声明和图标。Skills-only ZIP 用于门户的 Skills 上传；完整 ZIP 用于本地安装和 GitHub 分发。上传、提交、批准、发布是独立步骤，此说明不代表0.7.0已经完成这些步骤。
