...
Code Ontology Companion 0.7.0 随附以下浏览器库，使每个生成的工作台都保持为自包含的本地 HTML 文件。这些库不从 CDN 加载，不发起由应用启动的网络请求，也不要求 npm 安装。

默认 3D 空间地图使用浏览器内置 Canvas2D API，不新增第三方 3D、WebGL、动画、worker 或无障碍库。下面的 Cytoscape.js 和 ELK.js 用于可选平面视图。
