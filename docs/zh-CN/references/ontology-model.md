...
版本 0.5.3 保留原有 direct triple 和稳定 node/edge identity。每个 JSON edge 的附加 `evidence` array 条目具有稳定 `rule_id`、定性 `basis`（`direct_syntax`、`resolved_static`、`framework_semantic`、`name_heuristic`）和 `runtime_status`（`not_applicable`、`runtime_unknown`）。Source-derived 条目可包含仓库相对 `path`、`line_start`、`line_end`，有界 `limitations` 用于说明重要不确定性。
...
`ontology.ttl` 可以加载到兼容 RDF 1.1 的存储中，例如 Apache Jena、RDF4J、GraphDB 或 Stardog。加载和配置这些产品超出本插件 v0.5.3 的范围，并可能引入独立许可证、服务、端口或资源要求。
