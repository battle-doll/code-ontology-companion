...
| --- | --- |
| `apply-code-ontology` skill and `apply_workflow.py` | Current-conversation routing, actual initial lookup, same-snapshot reads, and authorized supported-change checkpoints; no persistent activation state |
...
Application distinguishes installed files, skill discovery, exposed MCP tools and
a verified CLI. A host-observed plan is not execution evidence. The helper can
perform Code CLI reads and authorized sync; other products use their own
available workflows and do not inherit Code permissions. Context storage needs
selected confirmed records and read-back; Contracts needs an actual selected
artifact. Unsupported language changes and incomplete handoffs remain visible.
...
Version 0.5.3 retains the legacy `source`, `target`, and `type` relationship
