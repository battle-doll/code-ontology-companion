...
버전 0.3.3은 최대 2 MiB인 regular `.java` 및 `.py` file을 읽으며 aggregate file-count 및 byte limit를 fail closed 방식으로 적용합니다. symbolic link나 Windows reparse point를 따라가지 않습니다. 일반적인 dependency, VCS, generated-output, IDE, virtual-environment, cache directory를 건너뜁니다.

credential, secret, token, private key, keystore 또는 `.env` configuration을 암시하는 이름의 file은 지원되는 extension을 사용해도 제외됩니다.

선택적 `runtime-binding` command는 추가로 명시적으로 지정한 최대 2 MiB의 regular local JSON 또는 Markdown file 하나를 읽고, 완전한 JSON object 또는 정확히 하나의 `policy-json` fence만 받습니다. 다른 configuration file을 탐색하거나 scan하지 않습니다.
...
명시적 승인 후 `runtime-binding`은 정확한 local JSON 또는 `policy-json` document 하나를 읽고 target repository 외부에 create-only canonical receipt 하나를 씁니다. graph는 policy identifier를 보존하지만 configured value는 보존하지 않습니다. policy file은 수정하지 않습니다. 정확한 v1 receipt는 policy-document hash를 포함할 수 없으므로 consumer가 사용 시점에 baseline 및 shadow condition을 다시 확인해야 합니다.
...
Codex는 요청된 workflow를 제공하기 위해 analyzer command output을 처리할 수 있습니다. 해당 platform processing에는 OpenAI의 적용 약관과 개인정보 처리방침이 적용됩니다. 버전 0.3.3은 remote data service를 호출하거나 generated artifact를 upload하지 않습니다.

## 해석

graph는 static evidence입니다. 선택적 AETHER receipt의 `runtimeEffective=true`는 known supplied-policy shadowing이 없는 production control-flow branch에 대한 frozen active-source reachability만 뜻합니다. branch 실행, order 제출, policy 안전성, profit 변화의 evidence가 아닙니다. Reflection, runtime bean condition, generated proxy, external configuration, dynamic import, monkey-patching, dependency injection container, generated code가 실제 runtime behavior를 바꿀 수 있습니다. 버전 0.3.3은 정확한 create-only mode-`0400` receipt를 macOS/POSIX에서만 생성합니다. 동등한 owner 및 permission semantic을 단언할 수 없으므로 Windows에서는 fail closed 처리합니다.
