...
버전 0.7.0는 최대 2 MiB인 regular `.java` 및 `.py` file을 읽으며 aggregate file-count 및 byte limit를 fail closed 방식으로 적용합니다. symbolic link나 Windows reparse point를 따라가지 않습니다. 일반적인 dependency, VCS, generated-output, IDE, virtual-environment, cache directory를 건너뜁니다.
...
Codex는 요청된 workflow를 제공하기 위해 analyzer command output을 처리할 수 있습니다. 해당 platform processing에는 OpenAI의 적용 약관과 개인정보 처리방침이 적용됩니다. 버전 0.7.0는 remote data service를 호출하거나 generated artifact를 upload하지 않습니다.
