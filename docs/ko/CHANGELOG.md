...
- 지원 소스 변경 뒤 허가된 갱신과 최신 상태·경고를 확인하고 미지원·혼합 언어 범위를 명시합니다.
- Context는 선택·확정된 기록과 재조회, Contracts는 선택한 실제 교환 자료 검증으로 제한합니다.
- 두 스킬의 패키지·추출 실행 검증을 추가합니다. 체크포인트 근거 검사 뒤 최신 상태와 스냅샷 일치를 다시 확인해 검사 중 발생한 변경을 놓치지 않습니다.

## 0.6.0 - 2026-09-05

- GPT-6 Astra 출시를 기념해 검색 지침을 간결하게 정리하고 사람용 3D 화면과 AI용 자료를 분리했습니다. 측정하지 않은 모델 성능 향상이나 공식 제휴는 주장하지 않습니다.
- 제한된 실제 소스 지도를 3D로 먼저 표시하고 구조·영향·변경, 카메라 포커스, 근거 선택, 딥링크, 키보드·텍스트 탐색과 안전한 평면 대체 보기를 제공합니다.
- 정확한 심볼 우선 검색, 구조 필터, 이어서 조회, 스냅샷 고정, 방향별 의존 경로, 노드·관계·근거 수정 비교를 추가했습니다.
- 근거 ID를 보존하고 가려진 타입명과 미해결 호출을 보수적으로 처리합니다. 안전한 파일 판독과 내용 검증 후 제한된 스냅샷·검색 인덱스를 재사용합니다.
- 원본 Code 참조와 Context용 불변 로케이터를 추가했습니다. 실제 스냅샷의 엄격한 Contracts draft 변환은 미지원이며 합성 호환 테스트와 구분합니다.
- 커밋한 공개 저장소 자체를 분석해 소스 리비전과 체크섬이 있는 공개 데모를 생성합니다. 비공개 등록 정보는 게시하지 않습니다.
- 추출·검색·화면 동작·상호운용 테스트와 재현 가능한 전체/스킬 전용 패키지를 갱신했습니다. 실제 Astra A/B 평가는 미측정입니다.

## 0.5.3 - 2026-08-29

- Manifest, skill trigger, agent prompt, submission metadata를 source-level
  architecture, dependency injection, static change impact, snapshot 비교와
  runtime·deployment·web research·교차 plugin 경계 중심으로 명확히 했습니다.
- 5개 README 첫 화면을 문제 중심의 설치/사용, 대표 요청, 핵심 경계로 개선하고
  기존 architecture, security, privacy, 다국어 상세 문서는 보존했습니다.
