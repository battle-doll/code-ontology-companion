...
Code Ontology Companion 0.7.0는 생성되는 각 workbench가 자체 완결형 local HTML file로 유지되도록 다음 browser library를 vendor합니다. CDN에서 불러오지 않고, application이 시작하는 network request를 만들지 않으며, npm 설치가 필요하지 않습니다.
