...
Code Ontology Companion 0.6.0는 생성되는 각 workbench가 자체 완결형 local HTML file로 유지되도록 다음 browser library를 vendor합니다. CDN에서 불러오지 않고, application이 시작하는 network request를 만들지 않으며, npm 설치가 필요하지 않습니다.

기본 3D 공간 지도는 브라우저 내장 Canvas2D API를 사용합니다. 새 3D, WebGL, 애니메이션, worker 또는 접근성 라이브러리를 추가하지 않습니다. 아래 Cytoscape.js와 ELK.js는 선택적 평면 대안에 사용됩니다.
