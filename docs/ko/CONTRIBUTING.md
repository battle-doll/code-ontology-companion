...
4. 최종 commit된 source state에서 등록된 self-ontology를 refresh하고, declared version-policy와 validated release-evidence event를 lineage에 append합니다.
5. final commit과 필수 CI check가 완료된 후에만 release tag를 생성합니다. 게시된 release tag를 이동하거나 교체하지 마십시오.
