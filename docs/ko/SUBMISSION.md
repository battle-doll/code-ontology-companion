...
- 버전: 0.7.0
- 후보 상태: 새 출시·제출 초안. 0.7.0은 아직 승인되거나 게시되지 않았습니다.
- 게시 이력: 2026-08-29 OpenAI Platform에서 0.5.3의 **Published** 상태를 확인했습니다. 이 기록은 0.7.0의 승인을 뜻하지 않습니다.
...
0.7.0은 기존 `manage-code-ontology`에 `apply-code-ontology` 스킬을 추가합니다. 현재 대화에서 기존 작업공간을 확인하고 상태·관련 검색 또는 영향 조회를 실제 실행합니다. 실제 노출된 MCP를 우선하며, 없으면 검증된 같은 번들 CLI를 사용합니다. 설치·스킬 노출·MCP 노출·CLI 실행 여부와 계획·실행 결과를 구분하고 기존 설정을 보존합니다.
...
생성 ZIP은 manifest, 스킬, 스크립트, 참조 문서, 라이선스, 고지, 아이콘을 포함합니다. Skills-only ZIP은 포털 Skills 업로드에, 전체 ZIP은 로컬 설치와 GitHub 배포에 사용합니다. 업로드·제출·승인·게시는 별도 단계이며 이 안내가 0.7.0의 완료를 뜻하지 않습니다.
