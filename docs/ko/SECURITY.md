...
- active-source graph reconstruction, production-path proof, 알려진 policy-shadow check, explicit authorization, create-only mode-`0400` publication을 거친 후에만 선택적 runtime-binding receipt를 생성합니다.
- deterministic analysis, workspace operation, workbench, MCP를 network-free로 유지하고 telemetry를 수집하지 않습니다.
...
- Codex host가 활성화한 경우 read-only stdio MCP process만 시작합니다.
- port를 열지 않고 filesystem path 대신 등록된 workspace ID를 받습니다.
- MCP write, refresh, install, delete, upload, execution tool을 노출하지 않습니다.
...
`runtimeEffective=true`는 알려진 supplied-policy shadowing이 없는 static production-branch reachability로 제한됩니다. runtime execution, order submission, policy safety, profit causation의 증명이 아니며, 모든 receipt에는 정확한 false authority가 포함됩니다.
