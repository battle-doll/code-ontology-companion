...
- 対応ソースの変更後に許可された更新・鮮度・警告を確認し、未対応や混在範囲を明示します。
- Context は選択・確認済み記録と再読み取り、Contracts は選択した実際の交換資料の検証に限定します。
- 両スキルのパッケージと展開実行を検証します。チェックポイントの根拠収集後に鮮度とスナップショットの一致を再確認し、検証中の変更を検出します。

## 0.6.0 - 2026-09-05

- GPT-6 Astra の公開を記念し、検索指示を簡潔にし、人向け 3D 表示と AI 向けデータを整理しました。未測定のモデル性能向上や公式提携は主張しません。
- 実際のソースに基づく有界 3D マップを既定とし、構造・影響・変更、カメラ移動、根拠選択、ディープリンク、キーボード・テキスト操作と平面の代替表示を提供します。
- 完全一致の優先検索、構造フィルター、ページング、スナップショット固定、方向付き依存経路とノード・関係・根拠の変更比較を追加しました。
- 根拠 ID を保持し、隠蔽された型名と未解決の呼出しを保守的に扱います。安全な読取と内容検証後に有界インデックスを再利用します。
- Code のネイティブ参照と Context 用の不変 locator を追加しました。実際のスナップショットの Contracts draft 変換は未対応で、合成互換性テストと区別します。
- コミット済みの公開リポジトリから、リビジョンとチェックサム付きの自己オン​​トロジーデモを生成します。非公開登録情報は公開しません。
- 抽出・検索・表示操作・相互運用テストと再現可能な配布パッケージを更新しました。実際の Astra A/B 評価は未測定です。
