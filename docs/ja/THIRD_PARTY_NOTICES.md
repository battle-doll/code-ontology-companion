...
Code Ontology Companion 0.6.0 は、生成される各ワークベンチを自己完結型のローカル HTML ファイルとして
維持するため、以下のブラウザーライブラリを同梱しています。これらは CDN から読み込まれず、
アプリケーションが開始するネットワークリクエストを行わず、npm のインストールも必要としません。

既定の 3D 空間マップはブラウザー内蔵の Canvas2D API を使用します。新しい 3D、WebGL、アニメーション、worker、アクセシビリティライブラリは追加しません。下記の Cytoscape.js と ELK.js は任意の平面表示に使用します。
