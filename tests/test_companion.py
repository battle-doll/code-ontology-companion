...
class CompanionTests(unittest.TestCase):
    ...
    def test_managed_snapshot_directory_link_fails_closed(self) -> None:
        self.initialize()
        snapshots = self.workspace / "snapshots"
        outside = self.base / "outside-snapshots"
        snapshots.rename(outside)
        snapshots.symlink_to(outside, target_is_directory=True)
        before = tree_digest(outside)

        operations = (
            lambda: companion.status(str(self.workspace), check_freshness=False),
            lambda: companion.history(str(self.workspace)),
            lambda: companion.query(str(self.workspace), "Demo"),
            lambda: companion.sync(str(self.workspace)),
        )
        for operation in operations:
            with self.subTest(operation=operation):
                with self.assertRaisesRegex(companion.CompanionError, "reparse point"):
                    operation()

        self.assertEqual(before, tree_digest(outside))

    @unittest.skipUnless(hasattr(os, "symlink"), "Symlink test requires platform support")
    def test_managed_staging_directory_link_fails_closed(self) -> None:
        self.initialize()
        outside = self.base / "outside-staging"
        outside.mkdir()
        (self.workspace / ".staging").symlink_to(outside, target_is_directory=True)
        (self.repo / "changed.py").write_text("def changed():\n    return True\n", encoding="utf-8")
    ...
    def test_record_preserves_evidence_class_and_exports_prov(self) -> None:
        ...
        recorded = companion.record(
            str(self.workspace),
            "decision",
            "Change declared retry-attempt limit from 2 to 3.",
            "declared",
            "OrderPolicy",
        )
